...
theme_update(
  ...
  plot.title = element_text(family = font_sans, size = 14),
  plot.subtitle = element_text(family = font_sans),
  plot.caption = element_text(family = font_sans, size = 8),
  axis.title = element_text(family = font_sans),
  axis.text = element_text(family = font_sans),
  legend.text = element_text(family = font_sans, size = 8)
)
...
ここでは、最新年度の手術あり・なし・合計の中で、主要病名をtop20で表示しています。
...
その上で今までの知識に頼らず、今回のグラフを時系列として読み取り、それぞれの経路がどのように変化しているかを分析してください。
...
散布図のラベルは年度となっており、時系列の変化を読み取ることができます。

この散布図は、以下の2つの指標を軸にしています。

-   **患者構成の指標**\
    全国の在院日数の平均を基準とし、以下の式で計算されます：

    ```         
    患者構成の指標 ＝（全国の在院日数の平均を使用した場合） ÷ 12.23
    ```

    → 値が **1より大きい** ＝ 重症度の高い疾患が多い構成\
    → 値が **1より小さい** ＝ 軽症の疾患が多い構成

-   **在院日数の指標**\
    全国平均の患者構成を用いて補正された各医療機関の在院日数との比であり、以下の式で計算されます：

    ```         
    在院日数の指標 ＝ 全国の在院日数の平均 ÷（全国の疾患構成に補正した在院日数）
    ```

    → 値が **1より大きい** ＝ 全国平均より在院日数が短い（＝効率的）\
    → 値が **1より小さい** ＝ 全国平均より在院日数が長い（＝非効率的）

    ### 散布図の4象限解釈

この2軸の交差によってできる散布図は、平均値（＝1.00）を基準に4つの象限に分かれます。それぞれの象限の解釈は以下の通りです。

#### 第1象限（横軸\>1, 縦軸\>1）

**「重症患者構成」かつ「在院日数短縮できている」**

-   高度急性期病院や大学病院など、高度な医療を必要とする患者が多い
-   それでも在院日数を短縮できており、プロセス効率・連携が良好
-   → **理想的な医療提供体制**

#### 第2象限（横軸\<1, 縦軸\>1）

**「軽症患者構成」かつ「在院日数短縮できている」**

-   平均的な地域病院や中小病院が含まれる
-   軽症中心ではあるが、効率的な運用がなされている
-   → **効率運用の成功例**
...
-   本来ならば早期退院が可能な構成だが、在院期間が長い
-   退院調整の遅れやベッド管理の非効率性が疑われる
-   → **改善余地のある医療機関**
...
-   医療資源を必要とする患者が多く、在院期間も長期化
-   がん末期や複数合併症のある高齢者なども想定される
-   → **構造的な理由がある可能性も含め、要検討の領域**
...
plot_data_animation <- filter_by_prefecture(data1_14, params$prefecture)

if(!is.null(plot_data_animation) && nrow(plot_data_animation) > 0) {
...
それぞれの病名の市区町村別の割合を表示しています。

ただしかなり煩雑なデータなので、ハルシネーションが起きやすいデータでもあります。
...
# 各手術タイプでtop20の病名を取得し、それぞれの病名について市区町村別グラフを生成
for(current_operation in operation_types) {
  cat(sprintf("\n\n## %s の Top 20 疾患\n\n", current_operation))
  
  # Top20の病名を取得
  top20_diseases <- DT_data2_8[
  ...
  # 各病名についてグラフを生成
  for(current_disease in top20_diseases) {
    cat(sprintf("\n\n### %s\n\n", current_disease))
